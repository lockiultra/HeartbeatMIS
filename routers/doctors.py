...
def add_new_doctor(doctor: schemas.DoctorCreate = Body(), db: Session = Depends(get_db)):
...
def update_doctor(doctor_id: int, doctor: schemas.DoctorUpdate = Body(), db: Session = Depends(get_db)):
    doctor_to_update = db.query(models.Doctor).filter(models.Doctor.id == doctor_id).first()
    for key, value in doctor.model_dump().items():
